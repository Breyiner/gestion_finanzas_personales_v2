...
export const estadosController = async (parametros = null) => {
    let containerTable = document.querySelector('.container-table');
    if (isAuthorize('statuses.index')) {
        const { data } = await get('statuses');
        containerTable.innerHTML = "";
        if (isAuthorize('statuses.store')) createBtnNew(containerTable);
...
function createBtnNew(container) {
    ...
    btnNuevo.setAttribute('id', 'crearEstado');
...
function crearTabla(container, data) {
    ...
    data.forEach(({ id, name }) => {
        ...
        rowBody.setAttribute('data-statusId', id);
        ...
        if (isAuthorize('statuses.update')) {
            ...
            btnEditar.setAttribute('data-statusId', id);
            btnEditar.setAttribute('id', 'editarEstado');
            btnEditar.textContent = "Editar";
            cellEditBody.append(btnEditar);
        }

        if (isAuthorize('statuses.destroy')) {
            const btnEliminar = document.createElement('button');
            btnEliminar.classList.add('boton', 'boton--rojo');
            btnEliminar.setAttribute('type', 'button');
            btnEliminar.setAttribute('id', 'eliminarEstado');
            btnEliminar.setAttribute('data-statusId', id);
...
async function eliminar(boton) {
    let dataId = boton.dataset.statusid;
    const confirmacion = await confirm("¿Está seguro de eliminar el estado?");
    if (confirmacion.isConfirmed) {
        const respuesta = await delet(`statuses/${dataId}`);
...
document.addEventListener('click', async (e) => {
    if (e.target.closest('#crearEstado'))
        location.href = `#/super_admin/estados/crear`;
    if (e.target.closest('#editarEstado')) {
        const id = e.target.closest('#editarEstado').dataset.statusid;
        location.href = `#/super_admin/estados/editar/status_id=${id}`;
    }
    if (e.target.closest('#eliminarEstado')) {
        await eliminar(e.target.closest('#eliminarEstado'));
    }
    if (e.target.closest('.modal-exit--estados')) cerrarModal();
});
